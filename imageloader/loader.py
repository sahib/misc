...
def parse_urllist(handle):
    'Parses a list of urls from the file descriptor pointed to by `handle`'
...
def basename_from_url(url):
    ...
    parsed = urlparse(url)
    *_, last = os.path.split(parsed.path)
    if len(last) == 0:
...
def save_image(basedir, url, content):
    ...
    data_path = os.path.join(basedir, 'images/objects', suffix)
    link_path = os.path.join(basedir, 'images/names', basename_from_url(url))

    try:
        os.makedirs(os.path.join(basedir, 'images/objects'), exist_ok=True)
        os.makedirs(os.path.join(basedir, 'images/names'), exist_ok=True)
...
def download_image(url, timeout=5):
    'Attempts to download the image at `url`, waiting at most `timeout` secs'
    try:
